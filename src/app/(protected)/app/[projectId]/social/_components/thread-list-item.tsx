...
export function ThreadListItem({
  ...
  return (
    <button
      onClick={onClick}
      className={cn(
        "w-full text-left p-3 sm:p-4 border-b hover:bg-accent/50 transition-colors",
        isActive && "bg-accent border-l-2 sm:border-l-4 border-l-primary"
      )}
    >
      ...
      <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
        <Badge
          variant="outline"
          className={cn(
            "rounded-lg px-1.5 sm:px-2 py-0.5 text-xs shrink-0",
            interest.className
          )}
        >
        ...
        <Badge
          variant="outline"
          className="rounded-lg px-1.5 sm:px-2 py-0.5 text-xs border-border/60 bg-background shrink-0"
        >
